...
public class Main {
	...
	public void manageControls() {
		...
		listFilesRepository = (TreeView) gladeFile.getWidget("lista_arquivos");
	}
	...
	public void manageEvents() {
		...
		menuConnect.connect(new MenuItem.Activate() {
			...
			public void onActivate(MenuItem arg0) {
				try {
					person = new Person();
					new Login(authentication, person, statusBar, fileChooserUpload, upload, download, listFilesRepository);
		...
		listFilesRepository.connect(new TreeView.RowActivated() {
			@Override
	...
	public void setListFiles() {
		...
		try {
			...
			listFiles = stub.retrieveAllFile();
		...
		//Transforma de Lista para Vector de File
        ...
        for (File file : files) {
            row = model.appendRow();
            model.setValue(row, fileName, file.getFilename());
            model.setValue(row, fileSize, String.valueOf(file.getSize()));
        }
        listFilesRepository.setModel(model);

        
        //Seta as propriedades das colunas
        column = listFilesRepository.appendColumn();
